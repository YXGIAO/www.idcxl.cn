...
func InitConfig() {
	AppConfig = Config{
		...
		DBHost:     getEnv("DB_HOST", "192.168.1.5"),
		DBPort:     getEnv("DB_PORT", "3306"),    // MySQL默认端口
		DBUser:     getEnv("DB_USER", "awa"),     // MySQL用户名，默认为root
		DBPassword: getEnv("DB_PASSWORD", "awa"), // MySQL密码，空字符串表示无密码
		DBName:     getEnv("DB_NAME", "awa"),     // MySQL 数据库名
